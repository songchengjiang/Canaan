...
namespace Canaan
{
    static Real MINOR(const Mat4& m, const size_t r0, const size_t r1, const size_t r2,
    ...
    Mat4::Mat4(Real m00, Real m01, Real m02, Real m03
    ...
    Mat4::Mat4(const Quat &rot)
    {
        makeRotation(rot);
    }

    Mat4::Mat4(const Vec3 &trans)
    {
    ...
    void Mat4::set(Real m00, Real m01, Real m02, Real m03
        , Real m10, Real m11, Real m12, Real m13
        , Real m20, Real m21, Real m22, Real m23
        , Real m30, Real m31, Real m32, Real m33)
    {
    ...
    void Mat4::makeRotation(const Quat &rot)
    {
        identity();
        Real fTx = rot.x + rot.x;
        Real fTy = rot.y + rot.y;
        Real fTz = rot.z + rot.z;
        Real fTwx = fTx * rot.w;
        Real fTwy = fTy * rot.w;
        Real fTwz = fTz * rot.w;
        Real fTxx = fTx * rot.x;
    ...
    void Mat4::makeTranslation(const Vec3 &trans)
    {
        identity();
        m[0][3] = trans.x;
        m[1][3] = trans.y;
    ...
    void Mat4::makeScale(const Vec3 &scl)
    {
        identity();
        m[0][0] = scl.x;
        m[1][1] = scl.y;
    ...
    void Mat4::makeLookAt(const Vec3 &eye, const Vec3 &center, const Vec3 &up)
    {
        Vec3 f = eye - center;
        f.normalize();
        Vec3 s = up.crossProduct(f);
        s.normalize();
        Vec3 u = f.crossProduct(s);
    ...
    void Mat4::makeOrtho(float left, float right, float bottom, float top, float zNear, float zFar)
    {
    ...
    void Mat4::makePerspective(float fovy, float aspectRatio, float zNear, float zFar)
    {
        float top = zNear * tan(DEGREE_TO_RADIAN * (fovy * 0.5f));
        float bottom = -top;
    ...
    void Mat4::decomposition(Vec3* position, Vec3* scale, Quat* orientation) const
    {
        ...
        if (scale || orientation){
            ...
            Real fInvLength = m3x3[0][0] * m3x3[0][0] + m3x3[1][0] * m3x3[1][0] + m3x3[2][0] * m3x3[2][0];
            if (fInvLength != 0.0f) fInvLength = 1.0f / sqrt(fInvLength);
            
            kQ[0][0] = m3x3[0][0] * fInvLength;
            kQ[1][0] = m3x3[1][0] * fInvLength;
            kQ[2][0] = m3x3[2][0] * fInvLength;
            
            Real fDot = kQ[0][0] * m3x3[0][1] + kQ[1][0] * m3x3[1][1] + kQ[2][0] * m3x3[2][1];
    ...
    Mat4 Mat4::transform(const Vec3& position, const Vec3& scale, const Quat& orientation)
    {
        Mat4 m;
